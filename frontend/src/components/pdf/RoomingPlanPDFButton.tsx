...
import RoomingPlanPDF, { type RoomingPlanPDFProps } from './RoomingPlanPDF';

interface RoomingPlanPDFButtonProps {
  planProps: RoomingPlanPDFProps;
  filename?: string;
}

export default function RoomingPlanPDFButton({
  planProps,
  filename = 'plan-chambres.pdf',
}: RoomingPlanPDFButtonProps) {
  const [generating, setGenerating] = useState(false);

  // ≠ PlanningPDFButton : PAS de cache du blobUrl — le rooming mute
  // (affectations), un blob caché serait périmé après un déplacement de
  // participant. Volume faible → régénération instantanée à chaque clic.
  const generate = async (): Promise<string> => {
    const { pdf } = await import('@react-pdf/renderer');
    const blob = await pdf(<RoomingPlanPDF {...planProps} />).toBlob();
