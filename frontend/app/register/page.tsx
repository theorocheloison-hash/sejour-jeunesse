...
const TYPES = [
  {
    ...
    title: 'Je suis organisateur de séjour',
    desc: 'Vous organisez un séjour collectif — voyage scolaire, classe verte, colonie de vacances, camp d’été, séjour d’association ou de mairie.',
    badge: 'Inscription gratuite',
  },
  {
    href: '/register/signataire',
    icon: (
      <svg className="w-8 h-8" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
        <path strokeLinecap="round" strokeLinejoin="round" d="M12 21v-8.25M15.75 21v-8.25M8.25 21v-8.25M3 9l9-6 9 6m-1.5 12V10.332A48.36 48.36 0 0 0 12 9.75c-2.551 0-5.056.2-7.5.582V21M3 21h18M12 6.75h.008v.008H12V6.75Z" />
      </svg>
    ),
    title: 'Je suis directeur(trice)',
    desc: 'Validez les dossiers de séjours scolaires de votre établissement. Signez les conventions et transmettez au rectorat.',
    badge: 'Rôle vérifié par LIAVO',
  },
  {
...
function RegisterContent() {
  ...
  if (type === 'director') {
    if (typeof window !== 'undefined') {
      window.location.replace('/register/signataire');
    }
    return (
      <main className="min-h-screen flex items-center justify-center bg-gray-50">
        <div className="h-6 w-6 animate-spin rounded-full border-2 border-[var(--color-border-strong)] border-t-transparent" />
