      ...
      <div>
        ...
        {devisAffiche && budgetData?.sejour && (() => {
          ...
          const pdfProps: DevisPDFProps = {
            ...
            titreSejour: s?.titre ?? d.sejourDirect?.titre ?? '',
            lieuSejour: s?.lieu ?? '',
            dateDebutSejour: s?.dateDebut ?? d.sejourDirect?.dateDebut ?? undefined,
            dateFinSejour: s?.dateFin ?? d.sejourDirect?.dateFin ?? undefined,
            natureSejour: (sejour.natureSejour ?? d.sejourDirect?.natureSejour) === 'EVENEMENT' ? 'EVENEMENT' : 'SEJOUR',
          ...
          // Devis « ouvert » : ni réellement signé (traces de signature) ni figé —
          // signable depuis l'espace quel que soit le mode (miroir de la garde backend).
          const devisOuvert = !d.signatureDirecteur && !d.nomSignataireDirecteur && !d.dateSignatureDirecteur && !d.signatureDocumentUrl
            && !['FACTURE_ACOMPTE','FACTURE_SOLDE','NON_RETENU','SIGNE_DIRECTION'].includes(d.statut);
          ...
          return (
            <div className="space-y-4">
              ...
              {(user.role === 'ORGANISATEUR' || apercuOrganisateur) && d.statut === 'EN_ATTENTE_VALIDATION' && (
