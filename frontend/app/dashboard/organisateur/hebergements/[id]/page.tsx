...
export default function HebergementDetailPage() {
  ...
  useEffect(() => {
    if (!authLoading && (!user || user.role !== 'ORGANISATEUR')) {
      router.push('/login');
    }
  }, [user, authLoading, router]);

  useEffect(() => {
    if (user?.role === 'ORGANISATEUR' && params.id) {
      setLoading(true);
      ...
      <nav className="bg-white border-b border-gray-200">
        <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex h-16 items-center justify-between">
            ...
            <Link href="/dashboard/organisateur/hebergements" className="text-sm text-gray-500 hover:text-gray-900 transition-colors">
              ← Retour au catalogue
            </Link>
          </div>
        </div>
