...
export default function ModifierSejourPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setSuccess(true);
      setTimeout(() => router.push('/dashboard/organisateur'), 1500);
    } catch (err) {
  ...
  if (sejour.statut !== 'DRAFT') return (
    <div className="min-h-screen bg-gray-50 flex items-center justify-center px-4">
      <div className="max-w-sm text-center">
        ...
        <Link href="/dashboard/organisateur" className="text-sm font-medium text-[var(--color-primary)] hover:underline">
      ...
      <nav className="bg-white border-b border-gray-200">
        <div className="max-w-2xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex h-16 items-center justify-between">
            ...
            <Link href="/dashboard/organisateur" className="text-sm text-gray-500 hover:text-gray-900 transition-colors">
              ← Retour
            </Link>
          </div>
        </div>
