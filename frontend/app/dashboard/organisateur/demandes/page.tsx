...
export default function OrganisateurDemandesPage() {
  ...
  useEffect(() => {
    if (user?.role === 'ORGANISATEUR') load();
  }, [user, load]);
      ...
      <nav className="bg-white border-b border-gray-200">
        <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex h-14 items-center gap-3">
            <Link href="/dashboard/organisateur" className="text-sm text-gray-500 hover:text-gray-900">&larr; Tableau de bord</Link>
          </div>
        </div>
      </nav>
