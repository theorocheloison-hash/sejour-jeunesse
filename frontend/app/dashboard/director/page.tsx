...
// ─── Modale Projet Pédagogique ──────────────────────────────────────────────

function DossierPedagogiqueModal({
  sejourId,
  titreSejour,
  onClose,
}: {
  ...
  useEffect(() => {
    getDossierPedagogique(sejourId)
      .then(setDossier)
      .catch(() => {})
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4" onClick={onClose}>
      <div className="bg-white rounded-2xl shadow-xl max-w-3xl w-full max-h-[90vh] flex flex-col" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="px-6 py-4 border-b border-gray-200 flex items-center justify-between">
          <div>
            <h2 className="text-base font-semibold text-gray-900">Projet pédagogique</h2>
            <p className="text-sm text-gray-500">{titreSejour}</p>
          </div>
            ...
            <>
              ...
              <div className="grid grid-cols-2 gap-4 text-sm">
                <div>
                  ...
                  <p className="font-medium">Du {fmtDate(dossier.dateDebut)} au {fmtDate(dossier.dateFin)}</p>
                </div>
                <div>
                  <p className="text-xs text-gray-500">Destination</p>
                  <p className="font-medium">{dossier.lieu}</p>
                </div>
                <div>
                  <p className="text-xs text-gray-500">Élèves</p>
                  <p className="font-medium">{dossier.placesTotales}</p>
                </div>
                <div>
                  <p className="text-xs text-gray-500">Hébergement</p>
                  <p className="font-medium">{dossier.hebergementSelectionne?.nom ?? '—'}</p>
                </div>
              </div>
              ...
              {dossier.thematiquesPedagogiques?.length > 0 && (
                <div>
                  <p className="text-xs font-semibold text-gray-500 uppercase tracking-wide mb-2">Thématiques pédagogiques</p>
                  <div className="flex flex-wrap gap-2">
                    {dossier.thematiquesPedagogiques.map((t, i) => (
                      <span key={i} className="rounded-full bg-[var(--color-primary-light)] px-3 py-1 text-xs font-medium text-[var(--color-primary)]">
                        {t}
                      </span>
                    ))}
                  </div>
              ...
              {dossier.accompagnateurs.length > 0 && (
                <div>
                  <p className="text-xs font-semibold text-gray-500 uppercase tracking-wide mb-2">Encadrement ({dossier.accompagnateurs.length})</p>
                  <div className="space-y-1">
                    {dossier.accompagnateurs.map((a) => (
                      <div key={a.id} className="flex items-center justify-between text-sm py-1 border-b border-gray-50">
                        <span className="text-gray-900">{a.prenom} {a.nom}</span>
                        <span className={`text-xs px-2 py-0.5 rounded-full ${a.signeeAt ? 'bg-green-100 text-green-700' : 'bg-orange-100 text-orange-700'}`}>
                          {a.signeeAt ? 'OM signé' : 'En attente'}
                        </span>
                      </div>
                    ))}
                  </div>
              ...
              <div>
                <p className="text-xs font-semibold text-gray-500 uppercase tracking-wide mb-2">
                  Autorisations parentales ({dossier.autorisations.filter(a => a.signeeAt).length}/{dossier.autorisations.length})
                </p>
                <div className="h-2 w-full bg-gray-200 rounded-full overflow-hidden mb-2">
                  <div
                    className="h-full bg-[var(--color-success)] rounded-full"
                    style={{ width: dossier.autorisations.length ? `${(dossier.autorisations.filter(a => a.signeeAt).length / dossier.autorisations.length) * 100}%` : '0%' }}
                  />
                </div>
              </div>
                          ...
                          <tbody>
                            {items.map((p) => (
                              <tr key={p.id} className="border-b border-gray-50">
                                <td className="py-1 pr-3 font-mono text-gray-500 w-24">{p.heureDebut}–{p.heureFin}</td>
                                <td className="py-1 text-gray-900">{p.titre}</td>
                                <td className="py-1 pl-3 text-gray-400">{p.responsable ?? ''}</td>
                              </tr>
                            ))}
                          </tbody>
                        </table>
        ...
        <div className="px-6 py-4 border-t border-gray-200">
          <button onClick={onClose} className="w-full rounded-lg border border-gray-300 px-4 py-2 text-sm font-medium text-gray-700 hover:bg-gray-50">
            Fermer
          </button>
        </div>
      </div>
      ...
      <div className="bg-white rounded-2xl shadow-xl max-w-2xl w-full max-h-[90vh] flex flex-col" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="px-6 py-4 border-t border-gray-200 flex justify-end gap-2">
          ...
          <button
            ...
            className="inline-flex items-center gap-1.5 rounded-lg border border-[var(--color-primary)] px-4 py-2 text-xs font-semibold text-[var(--color-primary)] hover:bg-[var(--color-primary-light)] transition-colors"
          >
        ...
        {showDossier && (
          <DossierPedagogiqueModal
            sejourId={detail.id}
            titreSejour={detail.titre}
