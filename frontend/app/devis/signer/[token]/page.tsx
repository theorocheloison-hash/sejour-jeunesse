...
export default function SignerDevisPage() {
  ...
  const [nomSignataireSigne, setNomSignataireSigne] = useState('');
  const [voirDevis, setVoirDevis] = useState(false);

  const [sentDirection, setSentDirection] = useState(false);
  const [emailDirecteurEnvoye, setEmailDirecteurEnvoye] = useState('');
  ...
  // Callbacks passés à SignatureDevisPanel (token-based). L'affichage d'erreur et
  // les états de chargement sont délégués au composant ; ici on relaie l'appel lib,
  // on capture le nom/email pour l'écran de succès et on re-fetch en silence.
  const handleSign = async (body: { nomSignataire: string; fonctionSignataire?: string; confirmation: true }) => {
    await signerDevisPublic(token, body);
    setNomSignataireSigne(body.nomSignataire);
    setSigned(true);
    // Re-fetch silencieux : récupère isSigned/nom/date pour la vue lecture seule.
    try { setDevis(await getDevisPublic(token)); } catch { /* garde l'état local */ }
  };

  const handleSendDirection = async (body: { emailDirecteur: string }) => {
    await envoyerDevisDirection(token, body);
    setEmailDirecteurEnvoye(body.emailDirecteur);
    setSentDirection(true);
  };

  const handleUpload = async (file: File) => {
    await uploadSignaturePublic(token, file);
    setUploaded(true);
      ...
      <div className="bg-white rounded-2xl border border-gray-200 shadow-sm p-8 max-w-md w-full text-center">
        ...
        <p className="text-sm text-gray-500">
          {uploaded
            ...
            : `Merci${nomSignataireSigne ? ` ${nomSignataireSigne}` : ''}. Votre réservation est confirmée. Un email de confirmation vous a été envoyé.`
          }
        </p>
        {devis?.centre && (
  ...
  if (sentDirection) return (
    <div className="min-h-screen flex items-center justify-center bg-gray-50 p-4">
      <div className="bg-white rounded-2xl border border-gray-200 shadow-sm p-8 max-w-md w-full text-center">
        ...
        <p className="text-sm text-gray-500">
          Un email a été envoyé à <strong>{emailDirecteurEnvoye}</strong> pour valider et signer le devis.
        </p>
      </div>
    </div>
  );
