...
Vérifie que les deux apps démarrent et que `npm test` (back) est vert **avant** toute modification. Colle les sorties dans le rapport §0. Si l'environnement ne démarre pas, tu **n'écris aucun code** : tu consignes et tu t'arrêtes.
...
Refondre les enums de statuts. Toucher S1, S3, S4, S5, S6 de l'analyse statuts. Toucher `verifyAccess`. Modifier des composants `Tab*` au-delà de la prop `mode` de `TabParticipantsCollab` en SC4. Réparer `soumettreAuRectorat`. Ajouter un tour modal / onboarding one-shot. Toucher au rendu hébergeur ou signataire. Renommer des routes API. Créer un second composant qui ressemble à un existant. Améliorer du code que tu croises sans l'avoir dans ton périmètre. Pousser.
