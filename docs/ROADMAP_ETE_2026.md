...
> 4. **Cascade suppression participant** — `deleteAutorisation` retire silencieusement l'élève de sa chambre (`AffectationChambre onDelete: Cascade`, schema:1809), SANS confirmation côté saisie directe (alors que « Retirer chambre » de TabChambres confirme). Ergonomie destructive. **Choix Théo = C** : noter, traiter comme brique jumelle après, ne pas gonfler le chantier A.
