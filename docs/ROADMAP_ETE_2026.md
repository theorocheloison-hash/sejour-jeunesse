...
> **État** : sous-chantiers 1+2 **LIVRÉS EN PROD 21/07** (étage 1 complet : alertes capacité + acquittement/réarmement par empreinte + sur-engagement backend). **Suite** : micro-run affichage `surEngagements` → 3 (référentiel backend) → 5 → 4 (le point dur — `syncOccupationsSejour` avec `STATUTS_DEVIS_RETENUS`, mise en transaction des 5 blocs signature) → 6 → 7 → 8. Chantier d'été, démontrable à Pierre/Tereva en septembre (Panier B #1).
