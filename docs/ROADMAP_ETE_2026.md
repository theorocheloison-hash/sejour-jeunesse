...
> 2. **GROUPES = brique jumelle du rooming** — l'affectation élève→groupe est `draggable={role==='ORGANISATEUR'}` + clôture, donc l'hébergeur-créateur ne peut PAS grouper ses élèves en propre (il crée déjà les groupes côté structure, seul le geste de remplissage manque). Même patron qu'A (élargir @Roles + `peutGererEnPropre` + drag), dans l'onglet Groupes. Périmètre chambres+groupes acté par Théo, mais **groupes traités À PART, après 3c**.
