...
> **État (22/07)** : EN PROD = étage 1 (alertes capacité + acquittement/réarmement + sur-engagement), référentiel back/front, **affectation backend 4a** (occupations + grille dispo + POST), **4b Lots 1-2** (transactionalisation des blocs signature publics), **cascades séjour** (re-datage sur changement de dates + suppression au soft-delete, `32e0279..6d43f2e`). **Suite** : **sélecteur chambres page séjour (#6, prochain)** — UI hébergeur d'attribution des chambres au séjour, consomme GET grille + POST occupations de 4a (⚠️ périmètre « frontend pur » vs « + endpoint batch » à trancher à la **lecture du controller** avant tout code) → **sous-chantier 7** (onglet Chambres collab, drag & drop participants, **rooming PDF spatial D13** — le livrable qui remplace l'Excel et se vend) → grille calendaire hébergeur (confort de pilotage, non bloquant démo) → **4b Lots 3-4** (blocs signature restants, session à froid) → recette croisée (#8). L'hygiène née du Lot 5 (injection inutilisée `SejourService`) se greffe au #7. Chantier d'été, démontrable à Pierre/Tereva en septembre **après recette réelle** sur le référentiel Sauvageon (26 ch./129 pl.).
