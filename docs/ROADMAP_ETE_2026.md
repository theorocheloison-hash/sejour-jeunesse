...
> 2. 🟠 **« Se désabonner » sur les emails TRANSACTIONNELS** (factures, devis) — le code ne pose AUCUN header `List-Unsubscribe` (vérifié `send()` : sender/to/replyTo/subject/html/attachment, rien d'autre) → header **injecté côté Brevo** à l'envoi. Risque réel : un client qui clique = adresse en **suppression list Brevo** → toutes ses factures suivantes non délivrées SILENCIEUSEMENT. ACTION ADMIN (pas code) : Brevo → paramètres transactionnels, chercher l'option List-Unsubscribe ; si absente → ticket support pour désactivation sur le flux transactionnel. ⚠️ Possible refus (exigence Gmail 2024 sur IP partagée) → dans ce cas le garde-fou = surveiller la suppression list Brevo périodiquement. Le désabonnement restera légitime UNIQUEMENT sur les futures campagnes marketing des hébergeurs (fonctionnalité pas encore existante).
