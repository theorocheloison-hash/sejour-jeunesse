...
> **État (23/07)** : EN PROD = étage 1 (alertes + acquittement/réarmement + sur-engagement), référentiel back/front, **4a** (occupations + grille + POST), **4b Lots 1-2** (blocs signature publics transactionnels), **cascades séjour** (`32e0279..6d43f2e`), **#6 onglet « Chambres » page séjour** (`2e63f93`+`00b4956` — UI hébergeur d'attribution, frontend pur ; recette LECTURE prod verte 26 ch. Sauvageon ; recette ÉCRITURE reportée septembre). **Suite** : **sous-chantier 7 (prochain)** — onglet Chambres de l'espace collab : **affectation participants→chambres** (backend POST/DELETE à CRÉER, capacité bloquante D7/D14) + drag & drop + **rooming PDF spatial D13** (le livrable qui remplace l'Excel et se vend) → grille calendaire hébergeur (confort, non bloquant démo) → **4b Lots 3-4** (blocs signature restants, session à froid) → recette croisée (#8). Hygiène Lot 5 (injection inutilisée `SejourService`) greffée au #7. Démontrable à Pierre/Tereva en septembre **après recette écriture réelle** (référentiel Sauvageon 26 ch./129 pl. déjà saisi).
