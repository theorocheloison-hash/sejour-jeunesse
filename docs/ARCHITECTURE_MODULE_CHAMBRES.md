...
> **Amendé le 22/07/2026** — validé par Théo : D13 précisée — rooming list = **noms par chambre, lits affichés en capacité** (le placement lit-par-lit reste un additif futur via `litId`, conditionné à une demande client — lève la tension avec le périmètre V1 « niveau chambre, lit optionnel ») ; **D14 ajoutée** (participants non signés). Vérification code du même jour : la ligne `AutorisationParentale` naît à l'inscription (CSV / saisie directe / unitaire), `signeeAt` seul marque la signature — la liste du drag & drop montre donc toujours la classe entière.
